...
author: "Croatian social media overview for Rimac+Nevera"
...
xiaomiD <- read.csv2("../dta/Rimac_full_csv.csv") %>%
  select(X.3) %>%
  row_to_names(.,1) %>%
...
xiaomi <- readxl::read_excel("../dta/Rimac_full.xlsx") %>% 
  row_to_names(.,1) %>% 
  clean_names() %>%
...
rijeci_clean %>%
  filter(word %in% c("rimac","nevera","prodaja")) %>%
  group_by(word) %>%
  count() %>%
...
<br>
```{r words, echo=FALSE, message=FALSE, warning=FALSE, eval = FALSE}

rijeci_clean %>%
  group_by(word) %>%
  count() %>%
  arrange(desc(n)) %>% slice(1:200) %>%
  ungroup() %>%
...
```{r brandOverTime, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 3, fig.width = 5, eval = FALSE}

brands <- c("rimac","nevera","prodaja")
...
rijeci_clean %>%
  ...
  summarise(n = n_distinct(id)) %>%
  arrange(desc(n)) %>%
  filter(n > 25) -> landscapeFreqBrand
...
site_data_columns <- read_excel("../dta/Rimac_full.xlsx",
                               sheet = "Facebook")
...
facebook <- readxl::read_excel("../dta/Rimac_full.xlsx",
                               sheet = "Facebook",
...
forums <- readxl::read_excel("../dta/Rimac_full.xlsx",
                               sheet = "Forums",
                               col_types = column_data_types) %>% 
  row_to_names(.,1) %>% 
  clean_names() %>%
...
portals <- readxl::read_excel("../dta/Rimac_full.xlsx",
                               sheet = "Portals",
                               col_types = column_data_types) %>% 
  row_to_names(.,1) %>% 
  clean_names() %>%
...
twitter <- readxl::read_excel("../dta/Rimac_full.xlsx",
                               sheet = "Twitter",
                               col_types = column_data_types) %>% 
  row_to_names(.,1) %>% 
  clean_names() %>%
...
yt <- readxl::read_excel("../dta/Rimac_full.xlsx",
                               sheet = "YouTube",
                               col_types = column_data_types) %>% 
  row_to_names(.,1) %>% 
  clean_names() %>%
...
press <- readxl::read_excel("../dta/Rimac_full.xlsx",
                               sheet = "Press",
                               col_types = column_data_types) %>% 
  row_to_names(.,1) %>% 
  clean_names() %>%
...
insta <- readxl::read_excel("../dta/Rimac_full.xlsx",
                               sheet = "Instagram",
                               col_types = column_data_types) %>% 
  row_to_names(.,1) %>% 
  clean_names() %>%
