...
datum <- read.csv2("../dta/Rimac_full_csv.csv") %>%
  select(X.3) %>%
  row_to_names(.,1) %>%
  mutate(Published = substr(Published,1,10)) #%>%
#mutate(Datum = as.Date(Published,"d%.m%.Y%"))

Datum  <- mutate(datum, b2= as.Date(Published, format= "%d.%m.%Y")) %>%
  select(-Published) %>%
  rename(Datum = b2)


rimac <- readxl::read_excel("../dta/Rimac_full.xlsx") %>% 
  row_to_names(.,1) %>% 
  clean_names() %>%
...
### Općenito
<br>
Podatci se odnose na sve medijske objave u RH u razdoblju od `r range(rimac$Datum)` i uključuju `r nrow(rimac)` objava.
<br>
<br>
**Koliko često korisnici spominju rimac u objavama**
<br>
<br>
Broj profila koji su objavili nešto o xiaomi-u: `r length(unique(rimac$author))`.<br>
Broj aktivnosti: `r length(unique(rimac$url))`.
...
<br>
<br>
**Kolika je raspršenost pojedinaca (profila) koji iznose svoja mišljenja i stavove o rimac (objavljuju li uvijek isti profili)**  
<br>
<br>
```{r, echo=FALSE, message=FALSE, warning=FALSE}
dta %>%
...
<br>
<br>
**Postoje li čvorišta iz kojih kreće u prosjeku relativno veći broj objava o rimac te definirati doseg objava iz čvorišta**
<br>
<br>
...
<br>
<br>
**Udio objava o rimac koji su izvorni sadržaj i udio objava koji su nastali kao dijeljenje već postojećeg sadržaja (share i retweet)**
<br>
<br>
...
**Atributi koji najčešće spominju u objavama u kojima se spominje rimac te prepoznati sentiment (pozitivno, negativno, neutralno) tih atributa**
<br>
<br>
...
```{r platforme, eval=FALSE, echo=FALSE, message=FALSE, warning=FALSE, fig.width=15, fig.height=10}
...
vazni_domena <- rijeci_clean %>%
  ...
  arrange(desc(n)) %>% #View()
  filter(word %in% c("menadžer", "uspjeh")) %>% #View()
